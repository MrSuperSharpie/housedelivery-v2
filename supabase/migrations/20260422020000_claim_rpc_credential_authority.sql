-- Wire check_credential_authority() into the claim RPC.
--
-- Changes from the 20260421010000 version:
--   • Flat discipline check (onboarding_status.disciplines) → replaced by
--     check_credential_authority(inspector, discipline, action)
--   • Flat credential_expires_at check → removed (authority function handles
--     per-credential expiry, insurance, safety rules)
--   • Required action is 'field_review' by default; upgraded to 'design_signoff'
--     when the job requires a professional seal
--
-- PRESERVED EXACTLY:
--   • Onboarding status gate (must be 'approved')
--   • Region check (regions are still on inspector_onboarding_status)
...
begin
  ...
  -- ── Onboarding gate (platform-level) ──────────────────────────────────────
  select *
  into v_eligibility
  ...
  -- ── Region check (still on onboarding_status, not in credential model) ────
  ...
  -- Determine the minimum authority action this job requires.
  -- Jobs requiring a professional seal need design_signoff authority;
  -- all others need field_review.
