-- Remove the validation_status check from claim_live_job_if_eligible.
-- The column was added in migration 20260407020000_project_job_validation.sql
-- which has not been applied to the live DB, causing:
--   "record v_job has no field validation_status"
-- Also retains the ::uuid casts from 20260409000000_fix_claim_rpc_uuid_cast.sql.
...
begin
  ...
  -- NOTE: validation_status check intentionally removed — column does not exist
