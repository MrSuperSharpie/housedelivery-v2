...
const templates: Record<VeroEmailEventKey, (context: VeroEmailTemplateContext) => VeroEmailTemplateContent> = {
  'inspector.application_submitted': context => ({
    ...
    body: [
      greeting(context, 'Thank you for submitting your inspector application to Vero Permit.'),
      'Your profile, credentials, and supporting documents are queued for review by the Vero Permit team.',
      'We will contact you if anything else is needed. Once review is complete, you will receive a clear platform access update.',
    ],
  ...
  'builder.profile_submitted': context => ({
    ...
    body: [
      greeting(context, 'Thank you for submitting your builder profile to Vero Permit.'),
      'Your account information is queued for review so your project records and inspection requests can be handled with the right controls.',
      'We will contact you if additional details are needed.',
    ],
