    ...
    <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center p-0 sm:p-4 pointer-events-none">
      ...
      >
        ...
        <div className="flex justify-center pt-3 pb-1 sm:hidden">
          <div className={cn('w-10 h-1 rounded-full', dark ? 'bg-slate-600' : 'bg-gray-200')} />
        </div>
        {/* Header */}
        {title && (
          <div className={cn('flex items-center justify-between px-5 py-4 border-b', dark ? 'border-slate-700' : 'border-gray-100')}>
            <h2 className="text-lg font-bold">{title}</h2>
            <button
              onClick={onClose}
              className={cn(
                'p-2 rounded-xl transition-colors focus:outline-none focus:ring-2 focus:ring-flame/30',
                dark ? 'text-slate-400 hover:bg-slate-700 hover:text-slate-100' : 'text-slate-500 hover:bg-slate-100 hover:text-slate-900'
              )}
            >
              <X className="w-5 h-5" />
            </button>
